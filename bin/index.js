...
function extractComments(filePath) {
    ...
    if (comments.length === 0) {
        console.log(chalk.yellow(`'${path.basename(filePath, path.extname(filePath))}' doesn't have comments to extract.`)); // Yellow color for no comments found
    }
    return comments;
}
...
function writeCommentsToMarkdown(comments, outputFilePath, sourcefile) {
    ...
    if (entrypointScss || consolidatedMd) {
        console.log(chalk.green(`'${sourcefile}' added to Markdown file successfully.`)); // Green color for successful file addition
    } else {
        console.log(chalk.green(`Markdown file '${outputFilePath}' updated successfully.`)); // Green color for successful markdown file update
    }
}
...
function processFile(filePath, outputFolder, consolidatedMd, entrypointScss) {
    ...
    if (allComments.length > 0) {
        const outputFilePath = path.join(outputFolder, consolidatedMd ? 'docs.md' : path.basename(filePath, path.extname(filePath)) + '.md');
        writeCommentsToMarkdown(allComments, outputFilePath, path.basename(filePath, path.extname(filePath)));
    }
...
function processFolder(folderPath, outputFolder, consolidatedMd, entrypointScss) {
...
function main(entryPath, outputFolder, consolidatedMd, entrypointScss) {
    if (entryPath === '.' || entryPath === './') {
        console.log(chalk.red("Error: Cannot process the current directory ('.'). Please specify a valid entry path.")); // Red color for error message
        return;
    }
